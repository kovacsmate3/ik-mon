...
class Storage implements IStorage {
    ...
    public function addUser($record): string {
        $id = uniqid();
        if (is_array($record)) {
            $record["id"] = $id;
        } else if (is_object($record)) {
...
class CardStorage extends Storage {
    public function __construct() {
        parent::__construct(new JsonIO('utils/cards.json'));
    }
    ...
    public function getCardsByOwner($owner_id) {
        return $this->findAll(['owner_id'=> $owner_id]);
    }
    
    public function getNumberOfCardsOwnedByUser($owner_id) {
        return count($this->getCardsByOwner($owner_id));
    }
    
    public function getRandomPurchasableCard($users) {
        $admin_id = ($users->getAdmin())['id'];

        $purchasableCards = $this->findAll(['owner_id' => $admin_id]);

        $numberOfPurchasableCards = count($purchasableCards);

        if ($numberOfPurchasableCards > 0) {

            $random_key = array_rand($purchasableCards, 1);

            return $purchasableCards[$random_key]['id'];
        }
...
class UserStorage extends Storage {
    public function __construct() {
        parent::__construct(new JsonIO('utils/users.json'));
    }
    ...
    public function getAdmin() {
        return $this->findOne(['username' => "admin"]);
    }
}
?>